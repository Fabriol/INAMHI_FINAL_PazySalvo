...
import math
import unicodedata
import pikepdf  # pyrefly: ignore [missing-import]
from flask import render_template  # pyrefly: ignore [missing-import]
from weasyprint import HTML, CSS  # pyrefly: ignore [missing-import]
...
# id="espejo_campo_<nombre>", y la celda que lo contiene termina con un
# marcador id="espejo_fin_<n>" (ver clase .ep-fin) que reporta su borde
# derecho real ya calculado por WeasyPrint — es lo que _inyectar_campos_editables
# usa para dimensionar cada casilla (ver ese borde geométrico más abajo).
...
# posición de la columna S/N pero con un dato real; con ancho fijo chico
# alcanza de sobra para 1-3 dígitos y nunca necesita calcular límite.
...
# Anchos AFM estándar de Helvetica (en 1/1000 em): la casilla que se dibuja
# a mano usa la fuente Type1 base14 /Helvetica (no una TTF embebida — este
# PDF no embebe ninguna fuente propia), así que esta tabla son las métricas
# REALES de la fuente que efectivamente se dibuja, no una aproximación. El
# BBox del Form XObject de cada casilla recorta (clip) cualquier trazo que
# se pase de su ancho, así que sin esta tabla un nombre largo en una columna
# angosta (ej. RESPONSABLE) se dibujaba a tamaño fijo y la mitad quedaba
# invisible fuera del recorte.
_ANCHO_HELV_1000 = {
        ...
        if es_badge:
            ...
            if _ancho_texto_pt(texto_dibujar, tam_fuente) > ancho_util:
                ...
                while recorte and _ancho_texto_pt(recorte, tam_fuente) + ancho_elipsis > ancho_util:
                    recorte = recorte[:-1]
                if recorte:
                    texto_dibujar = recorte.rstrip() + elipsis
                else:
                    # ni con "..." entra: probar solo el primer carácter (sin
                    # puntos suspensivos); si NI ESO entra, no dibujar nada.
                    # El BBox igual recortaría el trazo a la mitad, y un
                    # glifo cortado se ve como basura ilegible — mejor una
                    # casilla en blanco (rara, solo en celdas combinadas
                    # extremadamente angostas) que un carácter irreconocible.
                    primer_char = texto_dibujar[:1]
                    texto_dibujar = primer_char if _ancho_texto_pt(primer_char, tam_fuente) <= ancho_util else ''
...
def _ancho_campo(nombre_campo, x, ancho_pagina, limite_derecho=None):
    ...
    completa usan el borde derecho real de su propia celda o de su propio
    contenido (calculado en _inyectar_campos_editables a partir del
    marcador .ep-fin — ver ahí) como límite.

    El piso mínimo es chico (4pt) a propósito: cuando el límite viene de un
    marcador .ep-fin "propio" (campo que comparte celda con otro, ej.
    "Admin. Contrato: SI | Desc: ..."), el espacio real disponible puede ser
    genuinamente angosto. Forzar un piso más generoso (ej. 30pt, como tenía
    antes) pisaría la etiqueta o el campo vecino — mejor un valor muy
    encogido/truncado (ver _contenido_apariencia_campo) que invadir al de al
    lado. El respaldo "casi hasta el margen de la página" solo debería
    activarse si el template alguna vez agrega un campo sin su .ep-fin.
    """
    ...
    if limite_derecho is not None:
        return max(4, limite_derecho - x - 2)
    return max(60, (ancho_pagina - _MARGEN_PDF_PT) - x - 2)


def _recorrer_marcadores(pdf):
    """
    Devuelve, en el orden real en que aparecen en el documento, la posición
    (página, x, y) de cada marcador 'espejo_campo_<nombre>' (uno por cada
    campo editable, o guía) y 'espejo_fin_<n>' (uno por cada celda que
    contiene al menos un campo editable — ver .ep-fin en hoja_espejo.html).
    Estas posiciones vienen directo del layout ya calculado por WeasyPrint
    (bookmark-label: attr(id) en generar_documento_paz_salvo), así que
    reflejan bordes/columnas reales (colspan, padding, saltos de línea
    incluidos) en vez de tener que adivinarlos.
    """
    ...
    for item in recorrer(outline.root):
        if not item.title or not item.destination:
            continue
        if not (item.title.startswith('espejo_campo_') or item.title.startswith('espejo_fin_')):
            continue
...
def _inicializar_acroform_firmaec(pdf):
    """
    WeasyPrint no crea /AcroForm en el /Root. Crea el esqueleto vacío (solo
    /Fields) para que _inyectar_campos_editables tenga dónde agregar las
    casillas de datos.

    A propósito NO se fuerza aquí /SigFlags: forzarlo a 3 (SignaturesExist |
    AppendOnly) antes de que existiera una sola firma real hacía que el
    documento "prometiera" firmas que todavía no estaban, lo que coincide
    con el reporte de FirmaEC de "Documento sin firmas". pyhanko
    (PdfSigner.sign_pdf, ver subir_firma_pades en routes/paz_salvo.py) ya
    gestiona /SigFlags por su cuenta, de forma incremental, en el momento
    real en que agrega el primer campo de firma.
    """
    root = pdf.Root
    if '/AcroForm' not in root:
        root.AcroForm = pdf.make_indirect(pikepdf.Dictionary(Fields=pikepdf.Array([])))
    elif '/Fields' not in root.AcroForm:
        root.AcroForm.Fields = pikepdf.Array([])


def _inyectar_campos_editables(pdf, datos_diccionario):
    """
    ...
    Requiere que _inicializar_acroform_firmaec(pdf) ya se haya llamado sobre
    el mismo objeto 'pdf' (ambas corren dentro del mismo pikepdf.open()/save()
    — ver _construir_pdf_editable).
    """
    marcadores = _recorrer_marcadores(pdf)

    posiciones = {}
    for titulo, pagina, x, y in marcadores:
        if not titulo.startswith('espejo_campo_'):
            continue
        nombre_campo = titulo[len('espejo_campo_'):]
        if nombre_campo in CAMPOS_EDITABLES_ACROFORM:
            posiciones[nombre_campo] = (pagina, x, y)

    if not posiciones:
        return

    # Límite derecho de cada casilla: el siguiente marcador espejo_fin_ en
    # orden de documento (ver .ep-fin en hoja_espejo.html) — SIEMPRE, sin
    # comparar posiciones Y entre marcadores. Dos marcadores vecinos en la
    # MISMA línea visual pueden reportar un Y tan distinto como dos que
    # cayeron en líneas distintas (offsets propios de cada inline-block
    # anidado calculados por WeasyPrint) — comparar Y con un umbral fijo no
    # los distingue de forma confiable.
    #
    # Por eso cada campo que comparte celda con OTRO campo (ej. "Val: $X |
    # Acta: Y") trae su propio marcador anidado espejo_fin_own_N DENTRO de
    # su <span> (ver hoja_espejo.html): ese marcador reporta el borde
    # derecho real de SU PROPIO contenido, así que siempre es el próximo
    # espejo_fin_ que aparece en el documento después de él, sin importar si
    # el siguiente campo de la celda cae en la misma línea o en la de abajo.
    # El campo que SÍ es el último de su celda no tiene marcador propio: el
    # siguiente espejo_fin_ que encuentra es directamente el de la celda
    # completa (más ancho, porque no hay nada más a proteger a su derecha).
    limites_derecho = {}
    for i, (titulo, pagina, x, y) in enumerate(marcadores):
        if not titulo.startswith('espejo_campo_'):
            continue
        nombre_campo = titulo[len('espejo_campo_'):]
        if nombre_campo not in posiciones:
            continue
        for sig_titulo, sig_pagina, sig_x, sig_y in marcadores[i + 1:]:
            if sig_titulo.startswith('espejo_fin_'):
                if sig_pagina == pagina:
                    limites_derecho[nombre_campo] = sig_x
                break

    # Límite de CRECIMIENTO seguro para actualizar_campo_pdf_incremental
    # (ver /EPMaxX más abajo): SOLO se calcula para campos con marcador
    # propio (comparten celda con otro campo — el límite de arriba viene de
    # su fin_own_N, angosto porque solo protege el ancho del PLACEHOLDER
    # '-'/'0' que WeasyPrint dibujó al generar el PDF). Es el MÁS ANGOSTO
    # entre dos topes, porque cualquiera de los dos solo, por separado, es
    # insuficiente:
    #   (a) el marcador que aparece justo después de ese fin_own_N —
    #       normalmente el "_b" que antecede al SIGUIENTE campo de la MISMA
    #       celda (ver .ep-inl en hoja_espejo.html) — evita crecer encima
    #       del campo/etiqueta vecino inmediato.
    #   (b) el borde derecho REAL de toda la celda (el primer fin_ que NO es
    #       fin_own_, saltando los que haya de por medio) — evita salirse de
    #       la celda aunque (a) haya caído en un salto de línea con una X
    #       poco confiable (ver comentario de arriba sobre comparar Y).
    # Un campo sin marcador propio (el último/único de su celda) ya tiene el
    # ancho máximo real de su celda desde el principio: no necesita ni debe
    # crecer más.
    limites_crecimiento = {}
    for i, (titulo, pagina, x, y) in enumerate(marcadores):
        if not titulo.startswith('espejo_campo_'):
        ...
        for j in range(i + 1, len(marcadores)):
            if marcadores[j][0].startswith('espejo_fin_'):
                fin_idx = j
                break
        if fin_idx is None:
            continue
        fin_titulo, fin_pagina, _fin_x, _fin_y = marcadores[fin_idx]
        if fin_pagina != pagina or not fin_titulo.startswith('espejo_fin_own_'):
            continue

        candidatos = []
        if fin_idx + 1 < len(marcadores):
            sig_titulo, sig_pagina, sig_x, sig_y = marcadores[fin_idx + 1]
            if sig_pagina == pagina:
                candidatos.append(sig_x)
        for k in range(fin_idx + 1, len(marcadores)):
            k_titulo, k_pagina, k_x, k_y = marcadores[k]
            if k_titulo.startswith('espejo_fin_own_'):
                continue
            if k_titulo.startswith('espejo_fin_'):
                if k_pagina == pagina:
                    candidatos.append(k_x)
                break
        if candidatos:
            limites_crecimiento[nombre_campo] = min(candidatos)
    ...
    for nombre_campo, (pagina, x, y) in posiciones.items():
        ...
        limite_celda_x = limites_crecimiento.get(nombre_campo, x + ancho_campo)

        valor_actual = _valor_o_defecto(datos_diccionario.get(nombre_campo))

        # Inicialmente la apariencia es vacía para que se muestre el texto vectorial nítido
        # de WeasyPrint. Si se actualiza el campo de forma incremental después,
        # actualizar_campo_pdf_incremental creará un /AP con fondo blanco y el nuevo texto.
        contenido = b''
        ...
        widget = pdf.make_indirect(pikepdf.Dictionary(
            ...
            F=2,  # bandera "Hidden": oculto en pantalla e impresión (evita el resaltado azul)
            Ff=1,  # bandera "ReadOnly": nadie edita el campo a mano abriendo el PDF
            # Clave propia (no estándar de PDF, pero el spec permite entradas
            # privadas en cualquier diccionario): tope derecho seguro para
            # crecer, en pt de página (ver limites_crecimiento arriba).
            # actualizar_campo_pdf_incremental la usa para poder agrandar
            # /Rect más allá del ancho angosto de este widget (calculado
            # aquí solo para el placeholder inicial) sin arriesgarse jamás a
            # invadir el campo/etiqueta vecino ni salirse de la celda.
            EPMaxX=float(limite_celda_x),
        ))
...
def _construir_pdf_editable(ruta_pdf, datos_diccionario):
    """
    Punto único de escritura estructural: abre el PDF UNA sola vez, crea el
    esqueleto /AcroForm y agrega todas las casillas de campo editable, y
    guarda con una única reescritura completa (antes eran dos reescrituras
    pikepdf independientes y consecutivas). Menos revisiones de la
    estructura del documento antes de que pyhanko empiece a firmar de forma
    incremental reduce el riesgo de que su validador de xref choque con algo
    inesperado.

    object_stream_mode=disable: fuerza una tabla de referencias cruzadas
    clásica en vez de streams de objetos comprimidos (PDF 1.5+). pyhanko lee
    ambas formas sin problema, pero una tabla clásica es la estructura más
    ampliamente compatible para que un validador externo (FirmaEC) confirme
    la cadena de revisiones incrementales que vienen después.
    """
    with pikepdf.open(ruta_pdf, allow_overwriting_input=True) as pdf:
        _inicializar_acroform_firmaec(pdf)
        _inyectar_campos_editables(pdf, datos_diccionario)
        pdf.save(ruta_pdf, object_stream_mode=pikepdf.ObjectStreamMode.disable)
    ...
    with open(ruta_pdf, 'rb') as inf:
        ...
        for campo_ref in campos:
            ...
            if campo_obj.get('/T') == campo_formulario:
                ...
                x0 = float(rect[0]) if rect else 0.0
                y0 = float(rect[1]) if rect else 0.0
                y1 = float(rect[3]) if rect else _ALTO_CAMPO_EDITABLE_PT
                ancho_campo_actual = (float(rect[2]) - x0) if rect else 100.0
                alto_campo = (y1 - y0) if rect else _ALTO_CAMPO_EDITABLE_PT
                es_badge = campo_formulario in CAMPOS_SI_NO_COLOR
                es_fijo_corto = campo_formulario in CAMPOS_ANCHO_FIJO_CORTO

                if es_badge or es_fijo_corto:
                    # Los badges Sí/No y los campos de ancho fijo corto nunca
                    # necesitan crecer (ver _ancho_campo): su valor siempre
                    # cupo en el ancho fijo que ya tienen.
                    ancho_campo = ancho_campo_actual
                else:
                    # Este widget nació con /Rect angosto, ajustado al
                    # PLACEHOLDER que WeasyPrint dibujó al generar el PDF
                    # ('-' o '0' — ver _inyectar_campos_editables). Si el
                    # valor real que llega ahora es más largo, hay que
                    # agrandar la casilla o el texto queda ilegible
                    # (encogido al mínimo, ver _contenido_apariencia_campo).
                    # /EPMaxX es el tope derecho seguro para crecer
                    # (guardado al crear el widget, ver limites_crecimiento
                    # en _inyectar_campos_editables): crecer hasta ahí nunca
                    # invade al campo/etiqueta vecino ni se sale de la celda.
                    # Si /EPMaxX no existe (PDF generado antes de este
                    # cambio), no se crece — mismo comportamiento que antes,
                    # sin arriesgar nada.
                    max_x_obj = campo_obj.get('/EPMaxX')
                    max_x = float(max_x_obj) if max_x_obj is not None else float(rect[2])
                    ancho_disponible = max(ancho_campo_actual, max_x - x0 - 2.0)
                    tam_fuente_max = round(alto_campo * 0.62, 2)
                    ancho_deseado = _ancho_texto_pt(valor_texto, tam_fuente_max) + 4.0
                    # Nunca se encoge por debajo del ancho ya existente: ese
                    # ancho ya demostró cubrir con fondo blanco el texto
                    # nativo original de WeasyPrint; reducirlo dejaría un
                    # resto de ese texto visible fuera de la nueva casilla.
                    ancho_campo = min(max(ancho_campo_actual, ancho_deseado), ancho_disponible)

                if rect:
                    campo_obj['/Rect'] = generic.ArrayObject([
                        generic.NumberObject(x0), generic.NumberObject(y0),
                        generic.NumberObject(x0 + ancho_campo), generic.NumberObject(y1),
                    ])
                ...
                campo_obj['/F'] = generic.NumberObject(4)  # Hacerlo visible (Print) ahora que tiene un valor actualizado
...
def generar_documento_paz_salvo(solicitud, ex_funcionario, respuestas_db, ruta_salida):
    ...
    # El texto de cada campo (migrado o no) se deja visible/nítido tal como
    # lo dibuja WeasyPrint: la casilla de formulario que se inyecta después
    # nace vacía y oculta (ver _inyectar_campos_editables), así que no hay
    # nada que tape el texto nativo hasta que el campo se actualice de forma
    ...
    estilos_base = CSS(string=f'''
        ...
        * {{ -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }}
        ...
        /* Marcadores internos (invisibles) para ubicar cada celda de firma al momento de firmar,
           cada celda de dato editable (espejo_campo_) para convertirla en casilla de formulario
           PDF real, y el borde derecho real de esa celda (espejo_fin_, ver .ep-fin en
           hoja_espejo.html) para dimensionar esa casilla sin adivinar contra la celda vecina */
        [id^="espejo_firma_"], [id^="espejo_campo_"], [id^="espejo_fin_"] {{ bookmark-level: 1; bookmark-label: attr(id); }}
        /* position:relative: los campos que comparten celda con otro campo
           traen su propio marcador .ep-fin anidado (ver hoja_espejo.html,
           ej. espejo_fin_own_*) para reportar el borde derecho de SU PROPIO
           contenido, sin importar si el siguiente campo de la celda queda
           en la misma línea o se va a la de abajo. */
        span[id^="espejo_campo_"] {{ display: inline-block; position: relative; }}
    ''')
    ...
    # 6. Un solo pase de pikepdf: esqueleto /AcroForm + casillas de campo
    # editable para los campos ya migrados (Etapa 1): de aquí en adelante se
    # pueden seguir llenando de forma incremental, antes o después de
    # cualquier firma, sin regenerar este documento.
    _construir_pdf_editable(ruta_salida, datos_diccionario)
